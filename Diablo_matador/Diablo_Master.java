...
public class Diablo_Master extends TeamRobot 
{
	...
	private BulletsInfo_Master bulletsInfo = new BulletsInfo_Master(this);
		...
		if (this.isTeammate(e.getName()))
			...
			if (enemiesInfo.getRobot(e.getName()) != null) {
				...
				if (energyDiff > 0.0 && energyDiff <= 3.0) {
					bulletsInfo.addBullet(en, energyDiff);
					System.out.println("Enemy " + e.getName() + " shot");
				}
			}
			enemiesInfo.addRobotEvent(e);
		}
		broadcastMessageToDroids(e);
	}
	
	public void broadcastMessageToDroids(ScannedRobotEvent e) {
		RobotsInfo.Enemy en = enemiesInfo.getRobot(e.getName());
		Iterator<RobotsInfo.Enemy> it = friendsInfo.getMapIterator();
		while (it.hasNext()) {
			RobotsInfo.Enemy friend = it.next();
			double distance = RoboUtils.getRange(en.x, en.y, friend.x, friend.y);
			double bearing = RoboUtils.absbearing(en.x, en.y, friend.x, friend.y);
			ScannedRobotEvent event = new ScannedRobotEvent(e.getName(), e.getEnergy(), bearing,
					distance, e.getHeading(), e.getVelocity(), true);
			try {
				this.sendMessage(friend.name, event);
			} catch (IOException e1) {
				// TODO Auto-generated catch block
				System.out.println("IOEXCP in send message to: )" + friend.name);
				e1.printStackTrace();
			}
			distance = RoboUtils.getRange(this.getX(), this.getY(), friend.x, friend.y);
			bearing = RoboUtils.absbearing(this.getX(), this.getY(), friend.x, friend.y);
			event = new ScannedRobotEvent(this.getName(), this.getEnergy(), bearing,
					distance, this.getHeading(), this.getVelocity(), true);
			try {
				this.sendMessage(friend.name, event);
			} catch (IOException e1) {
				// TODO Auto-generated catch block
