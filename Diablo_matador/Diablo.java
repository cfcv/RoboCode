...
public class Diablo extends TeamRobot 
{
	...
	public void onMessageReceived(MessageEvent message) {
		...
		if (event != null) {
			RobotsInfo.Enemy sender = friendsInfo.getRobot(message.getSender());
			if (sender == null)
				return;
			if (!this.isTeammate(event.getName())) {
				enemiesInfo.addReceivedRobotEvent(event, sender);
				System.out.println("adding received enemy info");
			} else {
				if (event.getName() != this.getName())
					friendsInfo.addReceivedRobotEvent(event, sender);
			}
		}
	}
	...
	public void onScannedRobot(ScannedRobotEvent e){
		try {
			...
			System.out.println("fazendo broadcast do evento");
		} catch (IOException e1) {
			// TODO Auto-generated catch block
			System.out.println("exceção em broadcast message");
			e1.printStackTrace();
		}
